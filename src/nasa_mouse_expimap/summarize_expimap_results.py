...
def direct_tissue_summary(root: Path, tissue: str) -> dict:
    tissue_dir = root / f"outputs/expimap/runs/direct/{tissue}"
...
def reference_tissue_summary(root: Path, tissue: str) -> dict:
    tissue_dir = root / f"outputs/expimap/runs/reference_query/{tissue}"
...
def add_workflow_agreement(lines: list[str], root: Path, direct: dict[str, dict]) -> None:
    ...
    for tissue, summary in direct.items():
        ...
        direct_path = (
            root
            / f"outputs/expimap/runs/direct/{tissue}/raw_counts_nb_50epoch/analysis/flt_vs_gc_pathway_comparison.tsv"
        )
        reference_path = (
            root
            / f"outputs/expimap/runs/reference_query/{tissue}/query_nb_1000ref_50epoch/analysis/flt_vs_gc_pathway_comparison.tsv"
        )
