...
def direct_run_dir(tissue: str) -> Path:
    return Path(f"outputs/expimap/runs/direct/{tissue}") / "raw_counts_nb_50epoch"


def reference_root(tissue: str) -> Path:
    return Path(f"outputs/expimap/runs/reference_query/{tissue}")
...
def write_readme(variants: list[Variant], steps: list[Step], output_root: Path) -> Path:
    ...
    lines.extend(
        [
            ...
            "",
            "```bash",
            "python3 src/nasa_mouse_expimap/run_expimap_tissue_variant_matrix.py",
            "```",
            "",
