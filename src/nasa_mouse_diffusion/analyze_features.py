"""Analyze diffusion denoiser feature scores for FLT vs GC shifts."""
...
def feature_columns(frame) -> list[str]:
    return [column for column in frame.columns if str(column).startswith("DIFFUSION_FEATURE_")]
        ...
        rows.append(
            {
                ...
                "flight_minus_ground": float(np.nanmean(x) - np.nanmean(y)) if len(x) and len(y) else float("nan"),
...
def meta_analysis(effects):
    ...
    for feature, frame in effects.groupby("feature", sort=False):
        rows.append(
            {
                "feature": feature,
                **random_effects(frame["flight_minus_ground"].to_numpy(), frame["effect_variance"].to_numpy(), scipy_stats),
            }
        )
    result = pd.DataFrame(rows)
    direction_rows = []
...
def leave_one_accession_out(effects):
    ...
    for feature, frame in result.groupby("feature", sort=False):
        direction = float(require_import("numpy").sign(primary.get(feature, 0.0)))
        loo_direction = require_import("numpy").sign(frame["meta_effect"].astype(float).to_numpy())
        summary_rows.append(
            {
                "feature": feature,
...
def plot_pca(scores, features, output_dir: Path):
    ...
    if min(x.shape) < 2:
        return {}
    coords = sklearn_decomposition.PCA(n_components=2, random_state=0).fit_transform(x)
    frame = pd.DataFrame(
        {
            "PC1": coords[:, 0],
            "PC2": coords[:, 1],
    ...
    coord_path = output_dir / "diffusion_feature_pca.tsv"
    frame.to_csv(coord_path, sep="\t", index=False)
    paths = {"pca_coordinates": str(coord_path)}
    for color_col, name in [
        ("condition_inferred", "diffusion_feature_pca.png"),
        ("id.accession", "diffusion_feature_pca_by_accession.png"),
    ]:
        ...
        ax.set_xlabel("PC1")
        ax.set_ylabel("PC2")
        ax.set_title("Diffusion denoiser features")
...
def plot_umap(scores, features, output_dir: Path):
    ...
    if x.shape[0] < 5 or x.shape[1] < 2:
        return {}
    coords = umap_module.UMAP(n_components=2, random_state=0, n_neighbors=min(15, x.shape[0] - 1)).fit_transform(x)
    frame = pd.DataFrame(
        {
            "UMAP1": coords[:, 0],
            "UMAP2": coords[:, 1],
    ...
    coord_path = output_dir / "diffusion_feature_umap.tsv"
    frame.to_csv(coord_path, sep="\t", index=False)
    paths = {"umap_coordinates": str(coord_path)}
    for color_col, name in [
        ("condition_inferred", "diffusion_feature_umap.png"),
        ("id.accession", "diffusion_feature_umap_by_accession.png"),
    ]:
        fig, ax = plt.subplots(figsize=(7, 5))
        for label, group in frame.groupby(color_col, sort=True):
...
def plot_heatmap(scores, comparison, features, output_dir: Path, *, top_n: int):
    ...
    matrix = np.nan_to_num(scores.loc[order, selected].astype(float).to_numpy())
    ...
    ax.set_yticks([])
    ax.set_title("Top diffusion FLT vs GC feature shifts")
    fig.colorbar(im, ax=ax, shrink=0.75)
    fig.tight_layout()
    path = output_dir / "top_diffusion_feature_shift_heatmap.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    terms_path = output_dir / "top_diffusion_feature_shift_heatmap_features.tsv"
...
def write_readme(output_dir: Path, summary: dict, meta, loo_summary) -> None:
    ...
    for _, row in merged.head(10).iterrows():
        ...
        lines.append(
            f"| {row.feature} | {row.meta_effect:.4g} | {row.meta_fdr:.4g} | {int(same)}/{int(n_acc)} | {max_loo:.4g} |"
        )
...
def run(args) -> Path:
    ...
    missing = {"id.accession", "condition_inferred"}.difference(scores.columns)
    if missing:
        raise SystemExit(f"Scores missing required columns: {sorted(missing)}")
    features = feature_columns(scores)
    if not features:
        raise SystemExit("No diffusion feature columns found.")
    ...
    paths = {
        "comparison": output_dir / "flt_vs_gc_diffusion_feature_comparison.tsv",
        "per_accession_effects": output_dir / "per_accession_diffusion_feature_effects.tsv",
