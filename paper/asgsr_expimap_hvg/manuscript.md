...
Six models mapped 762 OSDR samples. Primary effects used 753 after exclusion of nine condition-strain-confounded spleen samples. The main positive set included 117 thymus samples (63 flight, 54 ground control; 5 projects), 151 skin samples (80, 71; 6 accessions collapsed to 4 mission projects for project-level checks), 197 liver samples (101, 96; 10 accessions collapsed to 9 projects), and 100 retained spleen samples (49, 51; 5 projects). The spleen model mapped 109 samples, but OSD-288 was excluded from the primary effect because its recorded strain labels were disjoint by condition. The secondary kidney model included 135 samples (68, 67; 6 projects). The supplementary soleus model included 53 samples (28, 25; 3 projects).
...
### Six tissue models provide a results-based positive core and explicit sensitivity set

The six models mapped 762 OSDR samples to 19,120 tissue-matched ARCHS4 profiles across 1,522 GEO series (Table 1; Fig. 1). The primary effect analyses used 753 samples after excluding OSD-288 spleen; the four positive-result tissues contributed 565 samples. HVG and pathway-membership filtering retained 1,975-1,997 genes and 319-387 Reactome programs per model. This provided hundreds of interpretable programs while keeping the input space comparable among tissues.
...
| Soleus | Supplementary sensitivity | 1,412 | 53 | 53 | 28 | 25 | 3 | 1,975 | 357 |

The original four-model package retains all 1,427 thymus, skin, liver, and soleus pathway records and a 153-pathway expanded family review (Tables S3 and S9-S11; Figs. S1-S3). The corrected kidney and spleen screen retains every pathway in Table S27 and every manually reviewed top-decile program in Table S28. Off-tissue labels, broad parent terms, seed reversals, nested terms, and latent scores contradicted by member genes were not promoted on magnitude alone. This review changed the tissue hierarchy: spleen replaced soleus in the positive core, kidney became an explicit secondary hypothesis, and soleus moved to supplementary sensitivity analysis.

### Orthogonal checks separate robust directions from statistical strength

Among the original primary top-decile programs, ssGSEA and preranked GSEA direction agreement was 86% and 86% in thymus, 77% and 87% in skin, 83% and 91% in liver, and 62% and 66% in soleus (Fig. S4; Tables S12-S14). Held-out-project direction prediction was 99%, 75%, 75%, and 28%, respectively. The soleus result was therefore weak under both method and project transfer. Complete retraining further narrowed the original primary-score narrative: 71% of reviewed thymus, 71% of skin, 67% of liver, and 57% of soleus programs retained one direction across all three runs.
...
### Soleus remains a supplementary negative sensitivity result

The soleus model did not identify a pathway that passed all five checks (Figs. S4-S9). Its largest immune and metabolic effects differed among OSD-104, OSD-714, and OSD-770; excluding condition-strain-confounded OSD-714 changed or attenuated several directions. Extracellular-matrix degradation changed direction across trainings and disagreed with conventional methods. Glycosaminoglycan metabolism and DNA repair accumulated method, project, and seed support but retained only 16% and 2% of their effects after composition adjustment. Training-only top pathways predicted held-out-project direction in only 28% of comparisons.

This negative result does not contradict the established soleus phenotype of atrophy, fiber-type transition, mitochondrial stress, and contractile change [24-30]. It shows that three heterogeneous bulk projects do not support a reproducible expiMap pathway mechanism under the current architecture. Soleus is therefore preserved as a transparent sensitivity result rather than omitted or used to anchor a positive claim.
