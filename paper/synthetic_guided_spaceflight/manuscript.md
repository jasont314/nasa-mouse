...
**Methods:** We assembled 1,610 mouse flight and ground-control bulk RNA-seq profiles through the NASA Open Science Data Repository API and audited all 997,515 profiles in ARCHS4 mouse. The framework varied expression transformations, feature spaces, harmonization, study scope, tissue structure, conditioning, and training regime. Paper-based WGAN-GP and diffusion implementations were screened with grouped validation, independent fidelity and effect-recovery gates, and locked testing; GeneJEPA was evaluated as a representation model because it has no expression decoder. The selected diffusion model was pretrained on 17,244 tissue-diverse ARCHS4 profiles. Downstream evaluation included a three-fold whole-accession analysis covering every tissue with at least three eligible studies. Each held-out accession was absent from OSDR adaptation, policy selection, and classifier fitting.

**Results:** None of nine matched liver harmonization arms passed all fidelity and conditional-effect gates. A calibrated study-conditioned WGAN-GP achieved correlation 0.976, precision 0.976, recall 0.994, and F1 0.985 on validation, but remained distinguishable from real data (adversarial accuracy 0.636) and failed accession-aware effect recovery; its locked test was not opened. The adapted diffusion model was the only generator to pass the final joint locked gates. Across four seeds, correlation was 0.974, precision 0.997, recall 0.996, F1 0.997, adversarial accuracy 0.475, and the Frechet-distance ratio was 0.074. The whole-study analysis evaluated 1,284 profiles from 63 unique accessions across 12 tissues. Synthetic-informed deployment changed accession-macro balanced accuracy from 0.581 to 0.583, AUROC from 0.620 to 0.631, and average precision from 0.694 to 0.708. Skin, pooled skeletal muscle, and lung met the predeclared tissue rule. Real-versus-synthetic FLT/GC effect correlation averaged 0.489 after tissue-specific random-effects aggregation but only 0.033 within individual accessions. A separate targeted thymus analysis prioritized a flight-lower mitotic panel, while soleus supplied a coherent developmental result involving lower *Bdh1*, *Ech1*, *Bnip3*, and *Decr1* with higher *Tpm1*.

**Conclusions:** Model choice depended on joint fidelity and biological-effect recovery rather than one favorable metric. Diffusion provided the strongest validated generator, but its value was tissue-dependent and modest across unseen studies. It recovered average tissue-level condition structure better than study-specific effects. Synthetic expression is therefore better treated as a feature prior or regularizer than as additional biological sample size. The thymus, soleus, skin, and pooled skeletal-muscle results define testable follow-up hypotheses rather than prospective validation.

**Keywords:** spaceflight; bulk RNA-seq; synthetic data; whole-study transfer; skeletal muscle; skin; thymus; NASA OSDR; ARCHS4
...
Fourth, whole-study transfer was tested in every canonical tissue with at least three eligible FLT/GC accessions. The 63 unique accessions were assigned to three global outer folds. Validation and test accessions were disjoint within each fold, and an accession had one role globally even when it contributed more than one tissue. Every one of the 68 tissue-accession pairs entered the real test set once, for a total of 1,284 profiles. The inner validation set chose a real-only baseline and the best synthetic-informed candidate. The generated candidate was deployed only if validation balanced accuracy improved by at least 0.02 and AUROC decreased by no more than 0.01; otherwise the fold reverted to its real-only baseline. A tissue passed the predeclared rule when this gate opened in at least two folds, aggregate balanced accuracy increased, and aggregate AUROC and average precision did not decrease. Earlier targeted accession-holdout experiments were retained as secondary context.

Flight-minus-ground effects were estimated within each OSDR study and then summarized with a random-effects model [17]. Generator recovery was reported at three levels. Pooled recovery compared one 974-gene real and synthetic effect vector after combining all test tissues in a fold. Tissue-specific recovery compared random-effects vectors for each tissue across its cross-fitted test accessions. Tissue-by-accession recovery compared real and synthetic vectors within each held-out study. These levels test broad condition coding, average organ response, and local study response, respectively, and are not interchangeable.
...
### Uniform whole-study transfer was tissue-dependent

The three-fold analysis evaluated every eligible tissue-accession pair once on real held-out profiles. Across the 63 unique accessions, validation-gated deployment changed macro balanced accuracy from 0.581 to 0.583, AUROC from 0.620 to 0.631, and average precision from 0.694 to 0.708. This was not a broad improvement in balanced classification. Skin, pooled skeletal muscle, and lung met the predeclared tissue rule, with positive changes in all three metrics and inner-gate support in at least two folds (Table 6; Fig. S5). Other tissues either reverted to real-only models in most folds or lost at least one aggregate metric.

Effect recovery depended on the aggregation level (Fig. S6). Pooled across tissues, the three fold correlations between real and generated FLT/GC vectors were 0.380, 0.158, and 0.160; mean direction agreement was 0.575. When accession effects were combined separately within each tissue, mean correlation increased to 0.489 and mean direction agreement to 0.682. Thymus, skin, and pooled skeletal muscle had tissue-level correlations of 0.776, 0.733, and 0.598. In contrast, direct tissue-by-accession correlation averaged 0.033 and direction agreement averaged 0.468 across 68 pairs. The generator therefore captured some average organ-level response structure but did not reproduce the local effect pattern of an arbitrary unseen study.

**Table 6. Three-fold whole-study transfer across all tissues with at least three eligible accessions. Deployment used the validation-selected synthetic-informed candidate when its inner gate passed and otherwise reverted to real-only training. Metrics are accession-macro averages on real test profiles.**

| Tissue | Accessions / profiles | Balanced accuracy, real to deployed | AUROC, real to deployed | Average precision, real to deployed | Rule passed |
|---|---:|---:|---:|---:|---|
| Adrenal gland | 3 / 39 | 0.626 to 0.626 | 0.704 to 0.704 | 0.756 to 0.756 | No |
| Brain | 4 / 57 | 0.524 to 0.549 | 0.589 to 0.485 | 0.654 to 0.561 | No |
| Cerebellum | 3 / 54 | 0.504 to 0.504 | 0.461 to 0.420 | 0.561 to 0.550 | No |
| Heart | 3 / 51 | 0.600 to 0.567 | 0.428 to 0.379 | 0.557 to 0.521 | No |
| Kidney | 6 / 133 | 0.495 to 0.495 | 0.392 to 0.421 | 0.496 to 0.523 | No |
| Liver | 12 / 239 | 0.566 to 0.562 | 0.658 to 0.584 | 0.731 to 0.694 | No |
| Lung | 3 / 76 | 0.447 to 0.503 | 0.433 to 0.596 | 0.524 to 0.605 | Yes |
| Retina | 4 / 76 | 0.441 to 0.427 | 0.345 to 0.407 | 0.530 to 0.562 | No |
| Skeletal muscle, pooled | 13 / 185 | 0.676 to 0.691 | 0.771 to 0.826 | 0.805 to 0.865 | Yes |
| Skin | 6 / 151 | 0.556 to 0.632 | 0.596 to 0.706 | 0.676 to 0.755 | Yes |
| Spleen | 6 / 108 | 0.557 to 0.517 | 0.669 to 0.688 | 0.736 to 0.749 | No |
| Thymus | 5 / 115 | 0.775 to 0.715 | 0.902 to 0.885 | 0.908 to 0.917 | No |
| All accessions, macro | 63 / 1,284 | 0.581 to 0.583 | 0.620 to 0.631 | 0.694 to 0.708 | 3 tissues passed |

### Targeted thymus analysis identifies lower proliferative renewal

A separately frozen targeted analysis used OSD-457 after its GEO series and every other OSDR-linked GEO series had been excluded from ARCHS4 pretraining; OSD-457 was also excluded from that experiment's OSDR adaptation and feature-policy development. Synthetic-guided gene selection changed balanced accuracy from 0.500 to 0.833 and AUROC from 0.840 to 0.979. The result held in both wild-type and Nrf2-knockout mice, and flight effects were closely aligned between the genotypes. This targeted result had been inspected before the final rerun and did not generalize as an average thymus utility gain in the uniform three-fold analysis, where deployed balanced accuracy decreased from 0.775 to 0.715. We therefore treat the panel as a real-supported biological follow-up rather than evidence that one thymus policy transfers generally.
...
|---|---|---|
| Thymus | Lower mitotic genes; APC/C, G2/M, and DNA replication | Targeted real-supported panel; uniform transfer did not improve average thymus classification |
...
| Lung | Uniform transfer improved all three predictive metrics across three accessions | Predictive transfer without a BH-FDR gene in the 974-gene panel |
...
Whole-study transfer was more selective than within-study development. Under one uniform three-fold protocol, skin, pooled skeletal muscle, and lung improved balanced accuracy, AUROC, and average precision, while the accession-macro gain across all tissues was small. The synthetic and real tissue-level meta-effect vectors were moderately concordant, but direct recovery within individual unseen accessions was weak. These results support synthetic data as a tissue-specific regularizer or feature prior, not as independent biological sample size or a replacement for study-aware inference.

In the targeted thymus analysis, the central genes already ranked strongly in real data. Synthetic guidance organized them into one interpretable cell-cycle panel after OSD-457 was removed from reference pretraining and OSDR adaptation. The uniform three-fold analysis did not improve average thymus classification, so this panel remains a targeted biological hypothesis. The development-screen labels describe which genes crossed repeated selection thresholds and are not claims of biological novelty.
...
The other tissues constrain the method's scope. Kidney combines one promoted and one reinforced gene but lacks pathway enrichment. Spleen combines strong within-study predictive gains with four BH-FDR genes, yet none survives the stricter LOO criterion. Skin promotes one directionally consistent gene while broader pathway enrichment remains heterogeneous; its positive uniform transfer result makes it a priority for a new study. Adrenal gland has only three studies. Retina has predictive and pathway gains without a synthetic-informed BH-FDR gene. Lung improved predictive transfer in the uniform analysis but had no BH-FDR gene in the 974-gene panel. Liver, quadriceps, and EDL selected real-only arms in development.

A new thymus study should test the fixed eight-gene panel prospectively. Soleus experiments should quantify the five-gene mitochondrial and contractile panel together with fatty-acid oxidation. Skin and pooled skeletal muscle should test frozen transfer policies in newly collected studies. Kidney should test *Inpp4b* and *Slc37a4* jointly in an unseen mission. Spleen should test *Rai14*, *Ptprk*, *Myl9*, and *Loxl1* in stroma-preserving assays, while *Igfbp3* should be evaluated separately as a real-data-derived stromal hypothesis.

### Limitations

No tissue currently has a prospective independent biological test. The uniform transfer analysis held out every eligible accession once, but the broader project had already inspected related OSDR outcomes before those folds were specified. OSD-457 was excluded from ARCHS4 pretraining, OSDR adaptation, and feature-policy development in the targeted thymus rerun, yet its outcome had also been examined earlier. Both analyses require replication in a newly collected study. Agreement across represented real studies supports focused follow-up but does not establish transfer to a new mission.
...
Finally, the generator represents tissues available during training but cannot encode the identity of a study it has never seen. The ARCHS4 reference failed its strict correlation-structure gate. After OSDR adaptation, the locked within-study test passed its finite-sample fidelity rule, but direct real-versus-synthetic effect correlation within held-out accessions averaged only 0.033 in the uniform transfer analysis. Tissue-level averaging recovered more structure, which can conceal study-specific disagreement. The model should not be assumed to reproduce a new mission, strain, or sample-processing protocol without additional testing. Exact model limitations, sensitivity analyses, and statistical safeguards are documented in the supplementary methods.

## Conclusions

Synthetic expression was useful only when its role was selected for the tissue and evaluated against real-only training. A pooled augmentation policy failed. In the uniform whole-study analysis, skin, pooled skeletal muscle, and lung met the transfer rule, but the gain across all accessions was small and local effect recovery was weak. The targeted thymus panel and the soleus development result provide coherent biological hypotheses, while kidney *Inpp4b* and *Slc37a4* remain focused secondary candidates.

Thymus and soleus supply the clearest process-level stories, but their evidence remains targeted or developmental. Skin and pooled skeletal muscle have the strongest combination of positive uniform transfer and synthetic-informed real-data findings. Lung has predictive transfer without a corresponding BH-FDR gene in the landmark panel. Kidney, spleen, adrenal gland, gastrocnemius, and tibialis anterior provide developmental hypotheses requiring unseen-study validation. Quadriceps, EDL, and liver do not support synthetic-guided claims because their real-only arms were retained.
