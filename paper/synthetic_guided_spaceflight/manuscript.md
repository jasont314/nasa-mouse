...
**Conclusions:** Model choice depended on joint fidelity and biological-effect recovery rather than one favorable metric. Among the tested models, diffusion had the best distributional validation metrics. Its downstream value still depended on tissue and mode of use. Synthetic expression worked better as a feature prior or regularizer than as additional biological sample size. The clearest biological programs appeared in thymus and soleus. Other tissues yielded narrower hypotheses that require independent replication.
...
The clearest biological programs appeared in thymus and soleus. Other tissue analyses yielded narrower candidates or negative results.
...
<p class="caption"><strong>Figure 5. Tissue evidence.</strong> (A) Repeated development-screen changes relative to real-only models for selected tissues. (B) BH-FDR genes promoted or reinforced by the synthetic workflow; synthetic labels are suppressed where the generated arm failed its metric gate. (C) Thymus and soleus provide the clearest process-level hypotheses; additional tissues provide narrower gene-level findings or negative results.</p>
...
The value of synthetic expression depended on choosing its role separately within each tissue and benchmarking it against real-only training. Pooled augmentation failed. Thymus and soleus produced the clearest process-level hypotheses. The remaining tissues produced narrower candidates, with no single secondary tissue taking priority.

Skin, spleen, pooled skeletal muscle, adrenal gland, gastrocnemius, and tibialis anterior yielded narrower developmental hypotheses. Lung and retina improved during development but lacked a corresponding synthetic-informed BH-FDR gene in the landmark panel. Quadriceps, EDL, and liver do not support synthetic-guided claims because their real-only arms were retained. Independent biological data are needed to test all of these hypotheses.
