...
For a prepared GLARE gene-list export:

```bash
conda run -n nasa-mouse env PYTHONPATH=src \
  python -m nasa_mouse_glare.metascape_client submit \
  --gene-lists <run-dir>/post_analysis/metascape_gene_lists/multiple_gene_lists.csv \
  --background <run-dir>/post_analysis/metascape_gene_lists/background.txt \
  --output-dir '<run-dir>/post_analysis/metascape_results/{session_id}' \
  --input-species 10090 \
